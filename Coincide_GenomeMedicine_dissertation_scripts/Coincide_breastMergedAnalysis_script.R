...
#feed into this one
saveDirGlobal <- "./breast_analysis"
saveDirMerged <- "./breast_analysis/mergedMatrix"
outputFile <- "./breast_analysis/clust_test_outMessages.txt"
library("Coincide")
#need to source a function tailored to analyzing specifically the curatedBreastData
#binary survival outcomes:
# TODO: fix this file or convert template to generic from ovarian
source("./Coincide_GenomeMedicine_dissertation_scripts/Coincide_metaFeaturesAnalysisWrapper-breastMerged.R")
...
## this code was already run in breastProcessAndGeneFeatures_script.R to help with selection of final datasets.
# output <- merge_datasetList(datasetList=dataMatrixList,
#                             minNumGenes = 10000,
#                             minNumPatients = 40,
#                             batchNormalize = c('none'))
# saveRDS(output,file=paste0(saveDirMerged, "/mergedExprMatrix_minVar001_17_studies_no_norm.rds",compress=TRUE))
output <- readRDS("breast_analysis/mergedExprMatrix_minVar001_17_studies_no_norm.RData.rds")
load("./Coincide_GenomeMedicine_dissertation_scripts/pam50Short_genes.RData")
...
kmeansConsensus <-  clustMatrixListWrapper(dataMatrixList,
                                           clustFeaturesList,
                                           clustMethod=c("km"),
...
Pam50_subtypes_noNorm <- assignCentroidSubtype(t(mergedNoNorm),minNumGenes=30,centroidRData="./Coincide_GenomeMedicine_dissertation_scripts/pam50_centroids_updatedSymbols.RData");
...
names(subtypeDF) <- c("sampleName", "cluster", "subtypeID", "subtype")
...
# Basal   Her2   LumA Normal
#    26     29     62     11 

attrDF_pam50Full <- data.frame(
                     c(rep.int(1,length(kmeansConsensus$clustSampleIndexList_PACR[[1]][[1]])),
                       rep.int(2,length(kmeansConsensus$clustSampleIndexList_PACR[[1]][[2]])),
                       rep.int(3,length(kmeansConsensus$clustSampleIndexList_PACR[[1]][[3]]))),
...
dataMatrixList <- readRDS(paste0(saveDirGlobal,"/curatedBreastData_dataMatrixList_proc_minVar001_min10kGenes_min40Samples.rds"))
output <- mergeDatasetList(datasetList=dataMatrixList,minNumGenes = 10000, minNumPatients = 40, batchNormalize = c('BMC'));
...
#we know these are strong clusters. have  minMeanClustConsensus = 0.8
...
output <- mergeDatasetList(datasetList=dataMatrixList,
                           minNumGenes = 10000,
                           minNumPatients = 40,
                           batchNormalize = c('combat'),
                           combatPvalueThresh=.1)
save(output,file=paste0(saveDirMerged, "/mergedExprMatrix_minVar001_17_studies_combat_norm.rds",compress=TRUE))

...
#GSEAanalysis=FALSE
metaFeaturesAnalysisWrapper <- function(metaFeatures,esets=NULL,
                                        CoINcIDE_output,
                                        clusterCoINcIDE_output,
                                        meanEdgePairPvalueThresh = .01,
                                        indEdgePvalueThresh = .05,
                                        minTrueSimilThresh = .4,
                                        maxTrueSimilThresh = Inf,
                                        nodeSizeScaleFactor=1,
                                        clustSizeThresh = 5,
                                        saveDir = saveDirMerged,
                                        experimentName = "ovarian_2000F",
                                        networkColors = "Set3",
                                        commMethod = "edgeBetween",
                                        minNumUniqueStudiesPerCommunity=3,
                                        minMedianNumEdgesPerNodeInCommunity=3,
                                        nodePlotSize=10,nodeFontSize=.7,
                                        ES_thresh = .5,
                                        ...
                                        ovarian=TRUE,
                                        fisherTestVariables = c("histological_type", "tumorstage", "recurrence_status","grade"),
                                        fisherTestVariableLegendNames=fisherTestVariables,
                                        fisherTestVariableTitleNames=fisherTestVariables,
                                        plotStackedYLimit=NA,minFractNN =.7,
                                        fractFeatIntersectThresh=.5,
                                        numFeatIntersectThresh =0,
                                        clustSizeFractThresh =0,
                                        findCommWithWeights=FALSE,
                                        plotSimilEdgeWeight = TRUE,
                                        plotToScreen=FALSE,
                                        refGeneListDir="~/GSEA_base_MSigDB_lists_merged.RData.gzip",
                                        minNumEdgesForCluster=1,
                                        fractEdgesInVsOutEdge=0,
                                        fractEdgesInVsOutComm=0
){
  ...
  commInfo$attrDF$size <- clustSizes[na.omit(match(as.numeric(as.character(commInfo$attrDF$clust)),
                                                   as.numeric(names(clustSizes))))]
  textOut <- capture.output(table(aggregateData$sampleClustCommKey$community,
                                  aggregateData$sampleClustCommKey$studyNum,exclude=FALSE))
  cat(gsub(" ","_","\nMeta-cluster number of samples by studies:\n"),textOut,sep="\n",append=TRUE,
      file=paste0(saveDir,"/",experimentName,"_metaclusterNumSamplesByStudies.txt"))
  cat(gsub(" ", "_", "\nMeta-cluster number of samples by studies:\n"),
      textOut,sep="\n", append=TRUE,file=summaryFile)
  networkStats <- advancedNetworkPlots(communityMembership=commInfo,
                                       brewPal = networkColors,
                                       saveDir=saveDir,
                                       clustIndexMatrix=clustIndexMatrix,
                                       plotToScreen=plotToScreen,
                                       experimentName=experimentName,
                                       plotEdgeWeight=plotSimilEdgeWeight,
                                       nodeSizeScaleFactor=nodeSizeScaleFactor)$network_stats
  
  
  write.table(networkStats,quote=FALSE,
              file=paste0(saveDir,"/",experimentName,"_networkStats_",Sys.Date(),".txt"))
  ...
  if(GSEAanalysis){
    ...
    ES_out <- computeMetaclustEffectSizes(metaClustSampleNames=binInfo$metaClustSampleNames,
                                          dataMatrixList=dataMatrixList,
                                          featureNames=metaFeatures$finalFeatures,
                                          minOtherClass=5,
                                          computeWilcoxon=FALSE)
    sigGenes <- selectMetaclustSigGenes(computeMetaclustEffectSizesOutput=ES_out,
                                        qvalueThresh=1,
                                        ESthresh=ES_thresh,
                                        includeWilcoxon=FALSE)
    
    summaryESPos <- summarizePosESMetaclustGenes(selectMetaclustSigGenesOut=sigGenes,
                                                 computeMetaclustEffectSizesOutput=ES_out)
    
    summaryESNeg <- summarizeNegESMetaclustGenes(selectMetaclustSigGenesOut=sigGenes,
                                                 computeMetaclustEffectSizesOutput=ES_out)
    #write to table:
    write.table(summaryESPos,
                file=paste0(saveDir,"/",experimentName,"_summaryGenes_ESpos_thresh_",ES_thresh,"_",Sys.Date(),".txt"),row.names=TRUE,quote=FALSE,col.names=TRUE)
   ...
   # for(g in 1:length(sigGenes$sigMetaclustGenesES_pos)){
    ...
    for(g in 1:length(GSEA_out_unique)){
      #what can't remove indices??
      write.table(as.character(unlist(GSEA_out_unique[[g]])),
                  file=paste0(saveDir,"/",experimentName,"_GSEA_uniqueSig_community_",g,".txt"),

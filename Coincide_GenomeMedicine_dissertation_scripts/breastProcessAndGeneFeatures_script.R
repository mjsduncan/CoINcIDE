...
saveDir_20 <- "./breast_analysis/metaRankWithTop20Genes"
saveDir_no20 <-  "./breast_analysis/metaRankNoTop20Genes"
outputFile <- "./breast_analysis/breast_proc_outMessages.txt"

###assumes this data is in your path. These are provided on Github:
load("./Coincide_GenomeMedicine_dissertation_scripts/pam50_centroids_updatedSymbols.RData")
...
#"~/R/x86_64-redhat-linux-gnu-library/curatedBreastData/data/curatedBreastDataExprSetList.rda"
...
# is error related to env link in assayData slot?
# curatedBreastDataExprSetList$study_1379_GPL1223_all@assayData
# <environment: 0x5636b1d687a0>

# try with processed eset from curatedBreastData package script
esets <- readRDS("breast_analysis/bcProcessedEsetList.rds")
...
# Dropping the following datasets: (none)
...
metaFeatures <- selectFeaturesMetaVariance_wrapper(dataMatrixList,rankMethod=c("mad"), 
                                                ...
                                                numTopFeatFromEachDataset=20,
                                                fractNATopFeatAllowedPerDataset=.1,
                                                selectMethod=c("median"),
                                                outputFile=outputFile)
...
message(paste0("Dropping the following datasets: ",paste(names(dataMatrixList)[metaFeatures$datasetListIndicesToRemove],collapse=" ")))
# Dropping the following datasets: 
...
message(paste0("Dropping the following datasets: ",paste(names(dataMatrixList)[metaFeatures$datasetListIndicesToRemove],collapse=" ")))
# Dropping the following datasets: (none)
saveRDS(metaFeatures, paste0(saveDir_no20, "/metaFeatures_200.rds"), compress = TRUE)
...
message(paste0("Dropping the following datasets: ",paste(names(dataMatrixList)[metaFeatures$datasetListIndicesToRemove],collapse=" ")))
# Dropping the following datasets: study_22226_GPL1708_all
...
for(d in 1:length(dataMatrixList)){
  tmp <- assignCentroidSubtype(t(dataMatrixList[[d]]),
                               minNumGenes=30,
                               centroidRData="./Coincide_GenomeMedicine_dissertation_scripts/pam50_centroids_updatedSymbols.RData")
